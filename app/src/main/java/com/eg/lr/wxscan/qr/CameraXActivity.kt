...
import com.eg.lr.wxscan.databinding.ActivityCameraxBinding
...
class CameraXActivity : AppCompatActivity() {
    ...
    private fun initWeChatQRCode() {
        ...
        Log.e(TAG, mWeChatQRCode.toString())
        Log.e(TAG, "Finish Init")
    }
    ...
    private fun bindCameraUseCases() {
        ...
        imageAnalyzer?.setAnalyzer(cameraExecutor, WeChatAnalyzer(mWeChatQRCode) { results ->
            runOnUiThread {
                if (results.isNotEmpty()) {
                    if (isAnalyzerActive) {
                        isAnalyzerActive = false
                        val data = Intent()
                        val content = results.first().toString()
                        Log.e("TAG", "扫码结果：$content");
                        data.putExtra(WxQrUtil.EXTRA_CONTRACT_DATA, content)
...
// Must unbind the use-cases before rebinding them
        ...
        try {
            ...
            Log.e("Use case binding failed", exc.message, exc)
        }
    }
    ...
    private fun getOutputDirectory(): File {
        val mediaDir = externalMediaDirs.firstOrNull()?.let {
            File(it, resources.getString(com.eg.lr.wxscan.R.string.app_name)).apply { mkdirs() }
        }
    ...
    private class WeChatAnalyzer(
        ...
        override fun analyze(image: ImageProxy) {
            Log.e(TAG, "size = ${image.width} * ${image.height}")
            Log.e(TAG, "rotationDegrees = ${image.imageInfo.rotationDegrees}")
